...
function makeOutput(completedTasks) {
    ...
    doc.pipe(fs.createWriteStream(path.join(__dirname, `${pdfWriteLocation}/test-canvas-theme-status-${now.year}-${now.month}-${now.day}-${now.hour}-${now.minute}-${now.second}.pdf`)));
