...
function makeOutput(completedTasks) {
    ...
    doc.pipe(fs.createWriteStream(`./PDFs/test-canvas-theme-status-${now.year}-${now.month}-${now.day}-${now.hour}-${now.minute}-${now.second}.pdf`));
...
async function start() {
    try {
        var output = await main('./screenshots', 'byui.test');
        makeOutput(output);
    } catch (error) {
        handleError(error);
    }
